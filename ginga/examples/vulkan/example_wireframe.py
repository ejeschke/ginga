#! /usr/bin/env python
#
# example_wireframe.py -- 3D plot (octahedron + wireframe sphere) with the
#                         Vulkan renderer's camera mode.
#
"""
Example of 3D plotting in Ginga using the **Vulkan** renderer.

Plots an octahedron within a wireframe sphere.  Scroll to zoom in/out, click
and drag to orbit.  This is the Vulkan counterpart of
``ginga/examples/gl/example_wireframe.py`` -- the same scene, drawn through the
GPU-native Vulkan renderer (``renderer='vulkan'``) driven by its 3D camera
(``renderer.mode3d = True``).

Requirements: Qt (PyQt6/PySide6), the ``vulkan`` binding + a device, numpy.
"""
...
from ginga.misc import log, Settings  # noqa
...
class Viewer:
    ...
    def __init__(self, app):
        ...
        self.top = app.make_window(title="Simple Ginga 3D Viewer (Vulkan)")

        # select the Vulkan renderer via the 'renderer' setting; the widget
        # displays the renderer's offscreen frame via the array path
        settings = Settings.SettingGroup(name='wireframe-vulkan',
                                         logger=self.logger)
        settings.set(renderer='vulkan')
        vw = CanvasView(self.logger, settings=settings, render='widget')
        vw.ui_set_active(True)
        self.vw = vw
        self.logger.info("renderer is: %s" % vw.renderer.kind)
...
# toggle 3D view (switches the Vulkan renderer to its camera transform)
