...
# — kept in sync with the same constant in the qtw / pgw / gtk4w
# wrappers.  The gtk3 backend records the field on the normalised
# column dict but doesn't act on it yet (phase 3 will wire it).
...
class WidgetBase(Callback.Callbacks):
    ...
    def set_bg(self, color):
        ...
        small per-widget ``CssProvider`` to the widget's style
        context with a class-scoped rule.  This means it works
        on transparent containers (``Gtk.Box``, ``Gtk.Grid``)
        too; the bg only shows in cell gaps where children don't
        paint their own bg.

        Themed widgets (Button, ComboBox, …) paint multi-state
        backgrounds; a generic ``set_bg`` may be partially
        overridden by hover / active states there."""
...
class TextArea(WidgetBase):
    ...
    def scroll_to_lineno(self, lineno, align='nearest'):
        ...
        it = buf.get_iter_at_line(lineno)
        vadj = self.widget.get_vadjustment()
...
class TreeView(WidgetBase):
    ...
    def _delete_subtree(self, level, shadow, model, spec, prune_empty):
        ...
        for key, sub in spec.items():
            ...
            if not sub or bnch.terminal:
                model.remove(bnch.item)       # removes the whole subtree
                del shadow[key]
                count += 1
            else:
                count += self._delete_subtree(level + 1, bnch.node, model,
                                              sub, prune_empty)
                if prune_empty and len(bnch.node) == 0:
                    model.remove(bnch.item)
                    del shadow[key]
                    count += 1
        return count
...
class TableView(TreeView):
    ...
    * ``scrolled(table, h_pct, v_pct)``
    ...
    def __init__(self, columns=None, show_header=True,
                 ...
                 dragable=False):
        # GTK fallback: cell modes degrade to row-equivalents on this
        # backend (no native rectangular-cell support in GtkTreeView).
        # The public API (get_selected_cells, copy/cut/paste callbacks)
        ...
        # those entries survive sort but are dropped when the
        # row's dict goes away (clear() / delete_row).  Initialised
        # BEFORE super().__init__ runs setup_table so the
        # cell-data-func closure installed there can safely
        # consult these dicts.
        ...
        # cells (phase 3 wires it on this backend).
    ...
    def _post_setup_columns(self):
        ...
        # Wire a single button-press handler the first time we get
        # here so per-row 'button'-widget cells can fire
        # ``cell_action`` on single click.  Idempotent: subsequent
        # rebuilds reuse the same handler.
        if not getattr(self, '_widget_btn_handler', False):
    ...
    def _apply_initial_colwidths(self):
        ...
        column descriptors.  Pixel integers only — string forms
        (used by the pgw backend's grid track syntax) are ignored
        here."""
    ...
    def delete_row(self, idx):
        ...
        # Capture the row dict's id before removing the row so we
        # can purge dangling colour entries that referenced it.
    ...
    def set_show_grid(self, tf):
        ...
        # GTK paints grid lines using ``gtk_render_line``, which
        # picks up the style context's ``border-color`` for
        # ``treeview.view``.  Most light themes set that at
        # ~rgba(0,0,0,0.1) — invisible on white.  Add (idempotently)
        # a stronger CSS rule so the lines actually show up.
    ...
    #
    # Implementation strategy: the cell-data-func already attached
    # to each column (via the parent's ``_mkcolfnN``) is invoked
    # by GTK on every paint of every cell.  We override the
    # factory to wrap the parent's value-setting logic with an
    # additional ``_apply_color_to_cell_gtk`` step that reads the
    # current colour state and sets the cell renderer's
    # ``foreground`` / ``foreground-set`` / ``cell-background`` /
    # ``cell-background-set`` properties.  GTK then paints the
    # cell with those props.  Mutators just update the storage
    # dicts and ``queue_draw`` the tree to force a fresh paint
    # pass — GTK calls the cell-data-func per cell, the func
    # consults the (now updated) dicts.
    ...
    # ----- widget-cell support (gtk3) -------------------------
    #
    # GTK has no real "widget per cell" facility — cells are
    # drawn by ``CellRenderer`` subclasses, not by hosted widgets.
    # We approximate by swapping the renderer per column based on
    # the column's ``widget`` field:
    #
    #   checkbox  → CellRendererToggle
    #   combobox  → CellRendererCombo
    #   progress  → CellRendererProgress
    #   button    → CellRendererText (bold label, click handled by
    #              a TreeView-level button-press hook)
    #
    # Per-row gates (``enabled_key`` / ``visible_key``) and the
    # widget cell value are applied in a custom cell-data-func.

    def _swap_widget_cell_gtk(self, tvc, model_col_idx, col):
        """Replace the column's default text renderer with the
        renderer appropriate for ``col['widget']`` and re-attach
        a widget-aware cell-data-func."""
        wtype = col['widget']
    ...
    def _make_widget_cell_gtk(self, col):
        ...
        if wtype == 'button':
            cell = Gtk.CellRendererText()
            # Bold text + centered to look button-ish (GTK has no
            # true button cell).
            cell.set_property('weight', 700)
            cell.set_property('weight-set', True)
            cell.set_property('xalign', 0.5)
    ...
    def _mk_widget_colfn(self, idx, kwd, col):
        """Cell-data-func for a widget cell.  Pushes the row's
        value onto the right cell property, applies fg/bg/bold
        colour overrides, and honours ``enabled_key`` /
        ``visible_key`` per-row gates."""
        wtype = col['widget']
        enabled_key = col.get('enabled_key')
        visible_key = col.get('visible_key')
        ...
        def fn(column, cell, model, iter, _data=None):
            ...
            if bg is None and self._alternate_row_colors:
                bg = self._alt_row_bg_for(model, iter)
            # ``foreground`` only applies to text-based renderers;
            # ``cell-background`` works for every renderer.
            if isinstance(cell, (Gtk.CellRendererText,)):
                if fg is not None:
                    cell.set_property('foreground', fg)
                    cell.set_property('foreground-set', True)
                    ...
                    cell.set_property('foreground-set', False)
                # weight (bold) — Pango 700 / 400.
                if bold is True:
                    cell.set_property('weight', 700)
                    cell.set_property('weight-set', True)
                elif bold is False and wtype != 'button':
                    # Button cells keep the bold styling we set in
                    # _make_widget_cell_gtk; an explicit non-bold
                    # row-color override would undo that look.
                    cell.set_property('weight', 400)
                    cell.set_property('weight-set', True)
            if bg is not None:
                cell.set_property('cell-background', bg)
    ...
    def _wire_widget_cell_signal_gtk(self, cell, col):
        """Connect the renderer's interaction signal to the
        framework callback (``cell_edited`` for checkbox/combobox
        — progress is read-only, button is wired separately via
        the TreeView's button-press handler)."""
        wtype = col['widget']
        col_key = col['key']
        if wtype == 'checkbox':
    ...
    def _row_dict_at_path_str(self, path_str):
        """Look up the row dict stored in column 0 of the model
        for the path string ``path_str`` (the form GTK callbacks
        receive)."""
        model = self.tv.get_model()
        if model is None:
            return None, None
    ...
    def _on_widget_toggle_gtk(self, cell, path_str, col_key):
        ...
        if d is None:
            return
        # Honour enabled_key (treat insensitive cells as no-op).
        ...
        d[col_key] = new_value
        self.tv.queue_draw()
        # Resolve the visible row index for the callback path.
        model = self.tv.get_model()
        path = model.get_path(it)
    ...
    def _on_tv_button_press_for_widget_cells(self, tv, event):
        """Handle clicks on cells whose column has
        ``widget='button'`` — fire ``cell_action(table, row_dict,
        col_key)``.  Other widget cells (toggle / combo) handle
        their own clicks through their renderer's native signal."""
        if event.button != 1:
            return False
        ...
        # Find the matching user column for this TreeViewColumn.
        cols = tv.get_columns()
        try:
            n = cols.index(tvc)
        except ValueError:
        ...
        if ekey is not None and not d.get(ekey, True):
            return False
        # Fire cell_action with the row dict (matches qtw).
    ...
    def _resolve_cell_color_gtk(self, row_id, col_key):
        """Walk cell → row → column → table layers, merging fg,
        bg, and bold channels independently across levels."""
        fg = bg = bold = None
    ...
    def _row_dict_at(self, idx):
        ...
        visible row ``idx``, or None if the index is out of range."""
        model = self.tv.get_model()
        if model is None:
            return None
...
class Expander(ContainerBase):
    ...
    def __init__(self, title=None, notoggle=False):
        ...
        if not notoggle:
            ...
            # The button's internal HBox (image + label) defaults to
            # centred inside the full-width toggle; halign=START on
            # that inner box pushes the arrow + title flush left to
            # match the gtk4 Expander styling.
            inner = toggle_w.get_child()
...
class Splitter(ContainerBase):
    ...
    def _get_pane(self):
        ...
        if self.orientation == 'horizontal':
            w = Gtk.HPaned()
            # NOTE: styling the separator doesn't seem to work in Gtk4
...
class Toolbar(ContainerBase):
    ...
    def add_spacer(self):
        sep_w = Gtk.SeparatorToolItem()
        sep_w.set_draw(False)
        sep_w.set_expand(True)
        sep = wrap(sep_w)
        self.widget.insert(sep_w, -1)
        self.add_ref(sep)
...
class Dialog(WidgetBase):
    ...
    def __init__(self, title='', flags=DIALOG_FLAGS_ONTOP, buttons=[],
        ...
        self.widget = Gtk.Dialog(flags=dialog_flags)
        self.buttons = []
        self.parent = parent
        self.modal = modal
        ...
        for name, val in buttons:
            btn_w = self.widget.add_button(name, val)
    ...
    def popup(self, parent=None):
        # An explicit parent passed here forces the transient-for
        # relationship; otherwise it is applied only when the on-top flag
        # is set (the default).
        force = parent is not None
        if parent is None:
            parent = self.parent
        if parent is not None and (force or (self.flags & DIALOG_FLAGS_ONTOP)):
