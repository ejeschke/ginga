...
from ginga.util import catalog, iohelper, loader
...
# Reference viewer
from ginga.rv.GenericControl import GenericShell
...
class GingaShell(GenericShell):
    ...
    def add_channel(self, chname, workspace=None,
        ...
        with self.lock:
            ...
            for spec in local_specs:
                opname = spec.setdefault('name', spec.get('klass', spec.get('module')))
            ...
            for spec in local_specs:
                if spec.get('start', False):
                    opmon.start_plugin_future(chname, spec.name, None)
    ...
    def build_toplevel(self, ignore_saved_layout=False):
        ...
        super().build_toplevel(ignore_saved_layout=ignore_saved_layout)
    ...
    def stop(self):
        # stop local plugins in every channel
        for chname in self.get_channel_names():
            channel = self.get_channel(chname)
            opmon = channel.opmon
    ...
    def _cursor_timer_cb(self, timer):
        ...
        """
        data = timer.data
        # NOTE: by doing a gui_do_oneshot, we cancel any previous calls
        # to _showxy() that are pending, thus we always get the latest info
        self.gui_do_oneshot('field-info', self._showxy,
    ...
    def _showxy(self, viewer, data_x, data_y):
        ...
        """
        if not hasattr(viewer, 'make_callback_children'):
