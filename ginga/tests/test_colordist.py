...
class TestColorDistMeta(type):
    '''Using a metaclass to populate the unittest class with the tests for each distribution,
instead of writing each one manually'''
    def __new__(mcs, name, bases, dict):
        ...
        for dist_name in dists.keys():
            ...
            dict['test_' + dist_name] = test_dist_function(dist_name)

        return type.__new__(mcs, name, bases, dict)

class TestColorDist(unittest.TestCase):
    __metaclass__ = TestColorDistMeta
